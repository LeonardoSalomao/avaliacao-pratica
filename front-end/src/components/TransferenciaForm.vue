...
export default {
  setup(props, { emit }) {
    ...
    const validarDataTransferencia = () => {
      ...
      const dataTransferencia = transferencia.value.dataTransferencia;
      ...
      if (!transferencia.value.dataTransferencia) {
        ...
        if (dataTransferencia < dataAtualFormatada) {
          erros.value.dataTransferencia =
            "A data de transferência não pode ser no passado.";
        } else {
          erros.value.dataTransferencia = "";
        }
      }
    ...
    const agendarTransferencia = async () => {
      ...
      try {
        ...
        erro.value =
          error.response?.data?.message || "Erro ao agendar transferência.";
      } finally {
        loading.value = false;
      }
    };
